...
@section('content')
    <section class="mx-auto max-w-5xl px-4 py-12">
        <h1 class="mb-2 text-3xl font-bold text-ink">Kontakt</h1>

        @if ($siteSettings->contact_intro)
            <div class="prose mb-8 max-w-2xl text-muted">{!! $siteSettings->contact_intro !!}</div>
        @else
            <p class="mb-8 text-muted">Skontaktuj się z nami — odpiszemy najszybciej jak to możliwe.</p>
        @endif

        {{-- Dane kontaktowe — karty w rzędzie --}}
        <div class="mb-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <a href="https://www.google.com/maps?q={{ urlencode($siteSettings->contact_address.', '.$siteSettings->contact_city) }}"
                ...
                class="group flex items-start gap-3 rounded-xl border border-gray-200 bg-white p-4 transition hover:border-brand/40 hover:shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2">
            ...
            @if ($siteSettings->contact_edelivery_address)
                <div class="flex items-start gap-3 rounded-xl border border-gray-200 bg-white p-4">
                    ...
                    <span class="min-w-0">
                        <span class="block text-xs font-bold uppercase tracking-wide text-muted">e-Doręczenia</span>
        ...
        @if ($siteSettings->contactBoxIsVisible())
            <div class="mb-10 rounded-xl border border-brand/30 bg-brand-light p-5">
        ...
        <div id="formularz" class="scroll-mt-24">

...
@section('content')
    ...
    <section class="mx-auto max-w-4xl px-4 py-12">
        ...
        <h1 class="mb-8 text-3xl font-bold text-ink">{{ $project->title }}</h1>

        @if ($schedulePage)
            <div class="mb-8 flex flex-col gap-3 rounded-lg border border-brand/20 bg-brand-light p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        @if ($showForWhom || $project->since)
            <p class="mb-8 flex flex-wrap items-center gap-x-6 gap-y-1 text-sm text-muted">
                @if ($showForWhom)
                    <span><i class="fa-solid fa-users mr-1.5 text-brand" aria-hidden="true"></i>{{ $project->for_whom }}</span>
                @endif
                @if ($project->since)
                    <span><i class="fa-solid fa-calendar-days mr-1.5 text-brand" aria-hidden="true"></i>{{ $project->since }}</span>
                @endif
            </p>
        @endif

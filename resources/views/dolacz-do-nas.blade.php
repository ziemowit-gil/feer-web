...
<div class="mx-auto max-w-5xl px-4 py-12">
    ...
    {{-- Nagłówek --}}
    <div class="mb-10">
        <h1 class="mb-2 text-3xl font-bold text-ink">{{ $title }}</h1>
        @if ($intro)
            <p class="max-w-2xl text-muted">{{ $intro }}</p>
        @endif
    </div>

    {{-- Metro kafelki (edytowalne z admina) --}}
    @if ($hubLinks->isNotEmpty())
        <ul class="mb-14 grid gap-5 {{ $hubLinks->count() === 2 ? 'sm:grid-cols-2' : ($hubLinks->count() === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-2 lg:grid-cols-4') }}" role="list">
